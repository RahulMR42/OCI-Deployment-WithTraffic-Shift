...
resource "oci_identity_group" "devops" {
  ...
  name           = "${var.app_name}_devops-group-${random_id.tag.hex}"
...
resource "oci_identity_user" "devopsuser" {
  ...
  name           = "${var.app_name}_devops-user-${random_id.tag.hex}"
}
...
resource "oci_identity_dynamic_group" "devopsgroup" {
  provider       = oci.home_region
  name           = "${var.app_name}_devopsdyngroup-${random_id.tag.hex}"
...
resource "oci_identity_dynamic_group" "runcmddynamicgroup" {
  provider       = oci.home_region
  name           = "${var.app_name}_run_cmd_dyn_group-${random_id.tag.hex}"
  description    = "run_cmd Pipeline Dynamic Group"
  compartment_id = var.tenancy_ocid
  matching_rule  = "ANY {instance.id = '${oci_core_instance.compute_instance-blue.id}',instance.id = '${oci_core_instance.compute_instance-green.id}'}"
}
