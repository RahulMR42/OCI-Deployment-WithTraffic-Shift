...
resource "oci_artifacts_repository" "test_repository" {
  ...
  display_name = "${var.app_name}-artifact-repo"
  is_immutable    = true
...
resource "oci_logging_log_group" "test_log_group" {
  compartment_id = var.compartment_ocid
  display_name   = "${var.app_name}_devops_log_group_${random_string.deploy_id.result}"
  defined_tags   = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}
...
resource "oci_ons_notification_topic" "test_notification_topic" {
  compartment_id = var.compartment_ocid
  name           = "${var.app_name}_${random_string.deploy_id.result}_devopstopic"
  defined_tags   = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}
...
resource "oci_devops_project" "test_project" {
  compartment_id = var.compartment_ocid
  name           = "${var.app_name}_project_${random_string.deploy_id.result}"
...
resource "oci_devops_deploy_environment" "test_deploy_instance_group_environment" {
  compute_instance_group_selectors {
    items {
      compute_instance_ids = [oci_core_instance.compute_instance-blue.id,
                              oci_core_instance.compute_instance-green.id]
...
resource "oci_devops_deploy_pipeline" "test_deploy_pipeline" {
  ...
  display_name = "${var.app_name}_devopspipeline"
...
resource "oci_devops_deploy_stage" "ts-demo" {
    deploy_pipeline_id   = oci_devops_deploy_pipeline.test_deploy_pipeline.id
    deploy_stage_type    = "LOAD_BALANCER_TRAFFIC_SHIFT"
    display_name         = "${var.app_name}_ts-swtich-to-new-env"
    freeform_tags        = {}
    defined_tags = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
    traffic_shift_target = "GREEN"

    blue_backend_ips {
        items = [
            oci_core_instance.compute_instance-blue.private_ip,
        ]
    }

    deploy_stage_predecessor_collection {
        items {
            id = oci_devops_deploy_stage.test_deploy_stage.id
        }
    }

    green_backend_ips {
        items = [
            oci_core_instance.compute_instance-green.private_ip,
        ]
    }

    load_balancer_config {
        backend_port     = 80
        listener_name    = "mr_listener_lb_2021-1210-1211"
        load_balancer_id = "ocid1.loadbalancer.oc1.iad.aaaaaaaaq4pjhmfb2pv37q36mq6daei4qrdlueza23qbbexisvtrkg2jogpa"
    }
...
resource "oci_devops_deployment" "test_deployment" {
  ...
  #Optional
  display_name = "${var.app_name}_devopsdeployment"
  defined_tags = { "${oci_identity_tag_namespace.ArchitectureCenterTagNamespace.name}.${oci_identity_tag.ArchitectureCenterTag.name}" = var.release }
}
